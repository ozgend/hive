...
public class BindableOptionsMenu extends View implements IMenuItemChangedCallback, IBindableView<BindableOptionsMenu> {
	...
	private Hashtable<Integer, AbsMenuBridge>	items		= new Hashtable<Integer, AbsMenuBridge>();
				...
				while (eventType != XmlResourceParser.END_DOCUMENT) {
					if (eventType == XmlResourceParser.START_TAG) {
						...
						if (id > 0) {
							...
							AbsMenuBridge item = AbsMenuBridge.create(nodeName, id, attrs, activity, model);
							if (item != null) {
								items.put(id, item);
							}
						}
	...
	public ViewAttribute<? extends View, ?> createViewAttribute(String attributeId) {
		try {
			...
			return (ViewAttribute<?, ?>) Class.forName(className).getConstructor(BindableOptionsMenu.class).newInstance(BindableOptionsMenu.this);
		} catch (Exception e) {
