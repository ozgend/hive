...
public abstract class Attribute<Th, T> extends Observable<T> {
	...
	/*
	 * @SuppressWarnings("unchecked") public final <To> void onPropertyChanged(IObservable<To> prop, Collection<Object> initiators){ set((T)prop.get(), initiators); }
	 */
	...
	protected void doSetValue(final T newValue, Collection<Object> initiators) {
		...
		/*
		 * uiHandler.post(new Runnable(){ public void run() { doSetAttributeValue(newValue); } });
		 */
	}
