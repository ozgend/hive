...
public class CollectionAdapter extends BaseAdapter implements CollectionObserver, Filterable, LazyLoadAdapter {
	...
	public CollectionAdapter(Context context, IModelReflector reflector, IObservableCollection<?> collection, Layout layout, Layout dropDownLayout, Filter filter, String enableItemStatement) throws Exception {
	...
	public CollectionAdapter(Context context, IModelReflector reflector, IObservableCollection<?> collection, Layout layout, Layout dropDownLayout, String enableItemStatement) throws Exception {
	...
	private View getView(int position, View convertView, ViewGroup parent, Layout layout) {
		...
		try {
			...
			if ((convertView == null) || ((mapper = getAttachedMapper(convertView)) == null)) {
				Binder.InflateResult result = Binder.inflateView(mContext, layout.getLayoutId(position), parent, false);
	...
	/**
	 * Make individual item enable/disable possible. This is not possible to do in Item level, but only from ListView's level since items are rendered by listView and listview
	 * seems to omit this value
	 */
