...
/**
 * User: =ra= Date: 08.10.11 Time: 11:58
 */
public class CursorCollection<T extends ICursorRowModel> extends ObservableCollection<T> {
	...
	public static interface ICursorCacheManager<ElType> {
		...
		/**
		 * Hint the Cache Manager the desire cache size needed, Size is not compulsory and implementations can choose it's own strategy to use this Most probably this size is the
		 * total visible item on screen
		 * 
		 * @param originator
		 * @param total
		 */
	...
	public CursorCollection(Class<T> rowModelType, IRowModelFactory<T> factory, ICursorCacheManager<T> cacheManager) {
		this(rowModelType, factory, cacheManager, null);
	}

	public CursorCollection(Class<T> rowModelType, IRowModelFactory<T> factory, ICursorCacheManager<T> cacheManager, Cursor cursor) {
		mRowModelType = rowModelType;
		mFactory = factory == null ? new RowModelFactory<T>(rowModelType) : factory;
		mCursor = cursor;
		mCacheManager = cacheManager == null ? new DefaultCursorCacheManager<T>() : cacheManager;
	...
	/**
	 * Default Implementation of CursorCacheManager. This will grow according to the number of widget accessing it, where size = (Sum of Visible Item on Each widget) * Extra
	 * Percentage and size is always larger than 10
	 * 
	 * @author andy
	 * 
	 * @param <T>
	...
	public static class DefaultCursorCacheManager<T extends ICursorRowModel> implements ICursorCacheManager<T> {
		...
		private WeakHashMap<Object, Integer>	cachingOriginators	= new WeakHashMap<Object, Integer>();
