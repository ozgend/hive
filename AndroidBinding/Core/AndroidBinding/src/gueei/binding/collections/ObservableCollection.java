...
public abstract class ObservableCollection<T> implements IObservableCollection<T> {
	...
	private WeakList<CollectionObserver>	mCollectionObservers	= new WeakList<CollectionObserver>();
	...
	@SuppressWarnings("rawtypes")
	public final void set(IObservableCollection newValue, Collection<Object> initiators) {
	}
	...
	/**
	 * Added: 18/10/2011 Set the number of visible children (most probably by Adapter) A collection might not bound to only one adapter, so this is for reference only
	 * 
	 * @param setter
	 *            the object the initiate this
	 * @param total
	 *            the count
	 */
