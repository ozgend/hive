...
public class ObservableMapper implements IPropertyContainer {
	...
	public void changeMapping(IModelReflector reflector, Object model) {
		...
		try {
			...
			/*
			 * for(String key: commandMapping.keySet()){ commandMapping.get(key).changeCommand(reflector.getCommandByName(key, model)); }
			 */
	...
	/*
	 * private class MockCommand implements Command{ private WeakReference<Command> command; public void Invoke(View view, Object... args) { if (command.get()!=null){ if
	 * (command.get() instanceof CollectionCommand){ ((CollectionCommand)command.get()).Invoke(mappedPosition); }else command.get().Invoke(view, mappedPosition, args); } } public
	 * void changeCommand(Command newCommand){ command = new WeakReference<Command>(newCommand); } }
	 * 
	 * public Command getCommandByName(String name) throws Exception{ if ((!commandMapping.containsKey(name)) && (mReflector!=null)){ MockCommand mCommand = new MockCommand();
	 * Command rCommand; rCommand = mReflector.getCommandByName(name, mModel); if (rCommand !=null){ commandMapping.put(name, mCommand); } } return commandMapping.get(name); }
	 */
