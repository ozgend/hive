...
public abstract class DependentCollectionObservable<T> extends Observable<T> implements CollectionObserver {
	...
	@Override
	public final void onCollectionChanged(IObservableCollection<?> collection, CollectionChangedEventArg args, Collection<Object> initiators) {
		dirty = true;
		changedArgs = args;
	...
	public T get() {
		if (dirty) {
			...
			try {
				...
				BindingLog.exception("DependentCollectionObservable.CalculateValue()", e);
			}
			dirty = false;
			changedArgs = null;
		}
