...
/**
 * User: =ra= Date: 08.10.11 Time: 12:05
 */
public class TrackedCursorCollection<T extends ICursorRowModel> extends CursorCollection<T> {
	...
	public TrackedCursorCollection(Class<T> rowModelType, IRowModelFactory<T> factory, gueei.binding.collections.CursorCollection.ICursorCacheManager<T> cacheManager, Cursor cursor) {
		super(rowModelType, factory, cacheManager, cursor);
	}

	public TrackedCursorCollection(Class<T> rowModelType, IRowModelFactory<T> factory, gueei.binding.collections.CursorCollection.ICursorCacheManager<T> cacheManager) {
	...
	/**
	 * There are no obvious methods like AddItem(s), RemoveItem(s), etc for Сursor data could be changed anywhere and anytime out from model sometimes we need to know about data
	 * changes Not sure if we have to track more than one uri (!!!)
	 * 
	 * @param context
	 *            Context to register for data changes
	 * @param uri
	 ...
	 *            If <code>true</code> changes to URIs beginning with <code>uri</code> will also cause notifications to be sent. If <code>false</code> only changes to the exact URI
	 *            specified by <em>uri</em> will cause notifications to be sent. If true, than any URI values at or below the specified URI will also trigger a match.
	 */
