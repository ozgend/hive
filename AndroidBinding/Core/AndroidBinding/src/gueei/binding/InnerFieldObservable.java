...
/**
 * This is an observable to help binding to sub-field of a given object For example: Suppose binding to "A.B.C" where A, B is observable and C is plain object Inner<A> observes B,
 * Inner<B> observes C When A change, the full path is regenerated, When B change, subpath from B is regenerated, leaving A unchange, but will notify A and A will notify its
 * observers. shit.. what am I writing?
 * 
 * @author andy
 * 
 * @param <T>
...
public class InnerFieldObservable<T> implements IObservable<T>, Undetermined {
	...
	public boolean createNodes(Object viewModel) {
		...
		if (field instanceof IObservable) {
			...
			BindingLog.warning("InnerFieldObservable.createNodes()", String.format("fieldname '%s' not found", fieldName));
			return false;
		} else {
