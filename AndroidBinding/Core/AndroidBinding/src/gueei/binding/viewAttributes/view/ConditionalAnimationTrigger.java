...
public class ConditionalAnimationTrigger extends AnimationTrigger implements Observer {
	...
	public ConditionalAnimationTrigger(IObservable<? extends Integer> id, IObservable<?> trigger, IObservable<TriggerType> type, IObservable<?> condition) {
		mId = id;
		mTrigger = trigger;
		mType = type;
	...
	private void evaluateAnimation() {
		...
		switch (Type) {
			case True:
				if (Boolean.TRUE.equals(TriggerValue))
					notifyAnimationFire();
				break;
			case Equal:
				if (TriggerValue.equals(TriggerCondition))
					notifyAnimationFire();
			case WhenLargerThan:
